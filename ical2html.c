...
static void print_header(struct icaltimetype start, struct icaldurationtype dur)
{
  ...
  printf("<style type=\"text/css\" media=\"print\"> div.page { page-break-after: always; page-break-inside: avoid; } </style>");
  printf("</HEAD>");
  ...
  for (y = start.year; y <= end.year; y++) {
    ...
    for (m = (y == start.year ? start.month : 1);
	 m <= (y == end.year ? end.month : 12); m++) {
      ...
      printf("<div class=\"page\"> <table><caption>%s %d</caption>\n", months[m], y);
      printf("<thead><tr><th class=header_sun>Sunday<th class=header_mon>Monday<th class=header_tue>Tuesday<th class=header_wed>Wednesday");
      printf("<th class=header_thu>Thursday<th class=header_fri>Friday<th class=header_sat>Saturday\n");
      printf("<tbody>\n");
	...
	day = icaltime_from_string(s);
	w = icaltime_day_of_week(day);
	if (w == 1) printf("<tr class=tr_%d>\n", w);
	printf("<td class=td_%d><p class=date>%d\n\n", w, d);
